...
function signatureCopy(actor:ManagedMailActor){
  ...
  if(actor==="system")return AI_SIGNATURE_SYSTEM_COPY;
  return AI_SIGNATURE_HUMAN_COPY;
}
...
// Sem actor explícito mantemos o comportamento histórico assistido. Nos envios reais
// mail-send-core passa sempre human/autonomy/system de forma explícita.
export function managedText(text:any,actor:ManagedMailActor="human"){
  const clean=stripManagedSignature(text);
  const body=clean?`${clean}\n\n—\n`:"";
  return `${body}${AI_SIGNATURE_TITLE}\n${signatureCopy(actor)}`;
}

export function managedHtml(text:any,actor:ManagedMailActor="human"){
  const clean=stripManagedSignature(text);
...
export function formatManagedEmail(text:any,actor:ManagedMailActor="human"){
