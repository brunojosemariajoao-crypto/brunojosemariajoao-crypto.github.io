...
import { appendActivity, getJson, setJson,acquireLease,remove } from "../ai-store.mts";
...
export default async () => {
  ...
  if(!(await acquireLease("worker/active-lease.json",15)))return;
  try{
    ...
    for(const thread of threads){
      ...
      if(!(operationMode!=="shadow"&&previous?.lastResult==="shadow")&&shouldSkip(previous,latestInboundId,runStarted)){skipped++;continue;}
  ...
  }finally{await remove("worker/active-lease.json");}
};
