...
function editOrder(id){
 ...
 openDrawer('Corrigir '+o.number,`<form id="editOrderForm"><label>Data <input type="date" name="deliveryDate" value="${esc(o.deliveryDate)}" required></label><div id="editOrderLines">${o.items.map(i=>`<div class="manual-line"><input name="quantity" value="${esc(i.quantity)}" placeholder="Qtd."><input name="unit" value="${esc(i.unit)}" placeholder="Unidade"><input name="product" value="${esc(i.product)}" placeholder="Produto"><input name="notes" value="${esc(i.notes)}" placeholder="Observações"><button type="button" data-remove-edit>×</button></div>`).join('')}</div><button type="button" id="addEditLine" class="btn ghost">Adicionar artigo</button><p class="error" id="editError"></p><button type="submit" class="btn primary">Guardar correção</button></form>`);
 const bind=()=>$$('[data-remove-edit]').forEach(b=>b.onclick=()=>b.closest('.manual-line').remove());bind();
 $('#addEditLine').onclick=()=>{$('#editOrderLines').insertAdjacentHTML('beforeend','<div class="manual-line"><input name="quantity" placeholder="Qtd."><input name="unit" placeholder="Unidade"><input name="product" placeholder="Produto"><input name="notes" placeholder="Observações"><button type="button" data-remove-edit>×</button></div>');bind();};
 $('#editOrderForm').onsubmit=async e=>{e.preventDefault();const b=e.currentTarget.querySelector('[type="submit"]');b.disabled=true;try{const items=$$('.manual-line',$('#editOrderLines')).map(row=>Object.fromEntries(['quantity','unit','product','notes'].map(k=>[k,$(`[name="${k}"]`,row).value.trim()]))).filter(i=>i.product);await api('/api/orders',{method:'POST',body:JSON.stringify({action:'edit_order',orderId:id,deliveryDate:e.currentTarget.elements.deliveryDate.value,items})});closeDrawer();await loadAll(true);toast('Encomenda corrigida.');}catch(err){$('#editError').textContent=err.message;}finally{b.disabled=false;}};
}
function composeOrder(id){
 const o=state.orders.find(x=>x.id===id);if(!o)return;
 openDrawer('Mensagem sobre '+o.number,`<form id="orderMessageForm"><p>Para: <strong>${esc(o.customerEmail||'Sem email')}</strong></p><label>Assunto<input name="subject" value="${esc('Encomenda '+o.number)}" required></label><label>Mensagem<textarea name="text" rows="8" required></textarea></label><p id="messageError" class="error"></p><button type="submit" class="btn primary" ${o.customerEmail?'':'disabled'}>Enviar mensagem</button></form>`);
 $('#orderMessageForm').onsubmit=async e=>{e.preventDefault();const f=e.currentTarget,b=f.querySelector('[type="submit"]');b.disabled=true;try{await api('/api/send',{method:'POST',body:JSON.stringify({to:o.customerEmail,subject:f.elements.subject.value,text:f.elements.text.value})});toast('Mensagem enviada.');closeDrawer();await loadAll(true);}catch(err){$('#messageError').textContent=err.message;}finally{b.disabled=false;}};
}
