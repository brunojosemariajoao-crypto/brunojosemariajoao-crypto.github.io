...
function itemText(i){return `${i.quantity??''}${i.unit?` ${i.unit}`:''} ${i.product||i.rawLine||''}${i.fulfillment==='unavailable'?' · INDISPONÍVEL':i.fulfillment==='pending'?' · A CONFIRMAR':''}`.trim();}
...
function bindDynamic(){
  ...
  $$('[data-edit-order]').forEach(b=>b.onclick=()=>editOrder(b.dataset.editOrder));
  $$('[data-compose-order]').forEach(b=>b.onclick=()=>composeOrder(b.dataset.composeOrder));
...
function editOrder(id){
 ...
 openDrawer('Corrigir '+o.number,`<form id="editOrderForm" class="order-form"><div class="form-grid">${[['customerName','Cliente'],['storeName','Loja'],['customerEmail','Email'],['customerPhone','Telefone'],['customerAddress','Morada'],['customerNotes','Observações do cliente']].map(([key,label])=>`<label class="field"><span>${label}</span><input name="${key}" type="${key==='customerEmail'?'email':'text'}" value="${esc(o[key]||'')}"></label>`).join('')}<label class="field"><span>Data da encomenda</span><input type="date" name="deliveryDate" value="${esc(o.deliveryDate)}" required></label></div><h3>Artigos</h3><div id="editOrderLines">${o.items.map(i=>`<div class="manual-line"><input name="quantity" value="${esc(i.quantity)}" placeholder="Qtd."><input name="unit" value="${esc(i.unit)}" placeholder="Unidade"><input name="product" value="${esc(i.product)}" placeholder="Produto"><input name="notes" value="${esc(i.notes)}" placeholder="Observações"><select name="fulfillment" aria-label="Disponibilidade">${[['accepted','Incluído'],['unavailable','Indisponível'],['pending','Por confirmar']].map(([value,label])=>`<option value="${value}" ${(i.fulfillment==='unavailable'?'unavailable':i.uncertain?'pending':i.fulfillment||'accepted')===value?'selected':''}>${label}</option>`).join('')}</select><button type="button" data-remove-edit>×</button></div>`).join('')}</div><button type="button" id="addEditLine" class="btn ghost">Adicionar artigo</button><p class="error" id="editError"></p><button type="submit" class="btn primary">Guardar correção</button></form>`);
 const bind=()=>$$('[data-remove-edit]').forEach(b=>b.onclick=()=>b.closest('.manual-line').remove());bind();
 $('#addEditLine').onclick=()=>{$('#editOrderLines').insertAdjacentHTML('beforeend','<div class="manual-line"><input name="quantity" placeholder="Qtd."><input name="unit" placeholder="Unidade"><input name="product" placeholder="Produto"><input name="notes" placeholder="Observações"><select name="fulfillment" aria-label="Disponibilidade"><option value="accepted">Incluído</option><option value="unavailable">Indisponível</option><option value="pending">Por confirmar</option></select><button type="button" data-remove-edit>×</button></div>');bind();};
 $('#editOrderForm').onsubmit=async e=>{e.preventDefault();const b=e.currentTarget.querySelector('[type="submit"]');b.disabled=true;try{const items=$$('.manual-line',$('#editOrderLines')).map(row=>Object.fromEntries(['quantity','unit','product','notes','fulfillment'].map(k=>[k,$(`[name="${k}"]`,row).value.trim()]))).filter(i=>i.product);await api('/api/orders',{method:'POST',body:JSON.stringify({action:'edit_order',orderId:id,...Object.fromEntries(['customerName','storeName','customerEmail','customerPhone','customerAddress','customerNotes','deliveryDate'].map(k=>[k,e.currentTarget.elements[k].value.trim()])),items})});closeDrawer();await loadAll(true);toast('Encomenda corrigida.');}catch(err){$('#editError').textContent=err.message;}finally{b.disabled=false;}};
}
...
function orderMessageDraft(o){
 const line=i=>[i.quantity,i.unit,i.product].filter(v=>v!==null&&v!==undefined&&v!=='').join(' ');
 const unavailable=(o.items||[]).filter(i=>i.fulfillment==='unavailable');
 const pending=(o.items||[]).filter(i=>i.fulfillment!=='unavailable'&&(i.uncertain||i.fulfillment==='pending'));
 const accepted=(o.items||[]).filter(i=>!unavailable.includes(i)&&!pending.includes(i));
 const parts=['Bom dia,'];
 if(o.status==='cancelled')parts.push('Informamos que a encomenda '+o.number+' foi cancelada.');
 else{
  parts.push('Recebemos a vossa encomenda '+o.number+(o.deliveryDate?' para '+dateOnly(o.deliveryDate):'')+'.');
  if(accepted.length)parts.push('Artigos registados:\n'+accepted.map(i=>'• '+line(i)).join('\n'));
  if(unavailable.length)parts.push('Artigos indisponíveis, não incluídos na encomenda:\n'+unavailable.map(i=>'• '+line(i)).join('\n'));
  if(pending.length)parts.push('Artigos ainda por confirmar:\n'+pending.map(i=>'• '+line(i)).join('\n'));
 }
 parts.push('Obrigado,\nVitalVeg');
 return parts.join('\n\n');
}
