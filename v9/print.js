...
const params=new URLSearchParams(location.search);let deliveryDate=params.get('date')||'';const orderId=params.get('id')||'';
...
function itemRow(i){return `<tr><td class="qty">${esc(i.quantity??'—')}</td><td class="unit">${esc(i.unit||'—')}</td><td><strong>${esc(i.product||i.rawLine||'—')}</strong>${i.fulfillment==='unavailable'?'<div class="warning">Indisponível — não confirmado</div>':''}${i.uncertain?'<div class="warning">Linha a confirmar</div>':''}</td><td class="notes">${esc(i.notes||'')}</td></tr>`;}
function orderSheet(o){return `<section class="order-sheet"><div class="order-head"><div><h2>${esc(o.storeName||o.customerName||o.customerEmail||'Cliente')}</h2><p>${esc(o.customerEmail||'')}</p></div><div class="number">ORDEM DE ENCOMENDA<strong>${esc(o.number||'—')}</strong></div></div><table class="items"><thead><tr><th>Qtd.</th><th>Unidade</th><th>Produto</th><th>Observações</th></tr></thead><tbody>${(o.items||[]).map(itemRow).join('')||'<tr><td colspan="4">Sem linhas de produto.</td></tr>'}</tbody></table>${(o.reviewReasons||[]).length?`<div class="warning">ATENÇÃO: ${esc(o.reviewReasons.join(' · '))}</div>`:''}</section>`;}
function preparationTable(summary){
  const rows=Array.isArray(summary?.rows)?summary.rows:[];
  if(!rows.length)return '<section class="prep-sheet"><h2>RESUMO DE ARTIGOS</h2><div class="empty compact">Sem artigos agregados.</div></section>';
  return `<section class="prep-sheet"><div class="prep-title"><div><h2>RESUMO DE ARTIGOS</h2><p>Totais calculados no servidor. Linhas duvidosas não entram silenciosamente na soma.</p></div><div class="prep-review ${summary.reviewRowCount?'has-review':''}"><strong>${esc(summary.reviewRowCount||0)}</strong><span>a confirmar</span></div></div><table class="prep-table"><thead><tr><th>Produto</th><th>Qtd. confirmada</th><th>Unidade</th><th>Clientes</th><th>Revisão</th></tr></thead><tbody>${rows.map(r=>{
    const notes=[];
  ...
  }).join('')}</tbody></table>${summary.reviewRowCount?'<div class="prep-alert">ATENÇÃO: existem linhas que não foram incluídas nos totais confirmados. Rever antes de confirmar a encomenda.</div>':'<div class="prep-ok">Todos os totais foram calculados apenas com linhas consideradas seguras.</div>'}</section>`;
}
...
async function load(){
  if(!deliveryDate&&!orderId){root.innerHTML='<div class="error">Falta a data de entrega.</div>';return;}
  try{
    const res=await fetch(orderId?`/api/orders?id=${encodeURIComponent(orderId)}`:`/api/orders?deliveryDate=${encodeURIComponent(deliveryDate)}`,{credentials:'include',headers:{'Cache-Control':'no-cache'}});
    ...
    const orders=(orderId?(data.order?[data.order]:[]):(data.orders||[])).filter(o=>o.status!=='cancelled');
    if(orderId&&orders[0])deliveryDate=orders[0].deliveryDate;
    ...
    root.innerHTML=`<header class="document-head"><div class="brand"><img id="originalLogo" alt="VitalVeg" hidden><div id="brandFallback" class="brand-fallback"><strong>VitalVeg</strong><span>Central Inteligente de Operações</span></div></div><div class="doc-meta"><h1>ENCOMENDAS</h1><p>Entrega: <strong>${esc(dateLabel(deliveryDate))}</strong></p></div></header><div class="summary-line"><span><strong>${orders.length}</strong> encomendas</span><span><strong>${lineCount}</strong> linhas de produto</span><span><strong>${esc(prep.rowCount||0)}</strong> totais de artigos</span><span>Gerado em ${esc(ptStamp.format(new Date()))}</span></div>${orderId?'':preparationTable(prep)}${orders.length?orders.map(orderSheet).join(''):'<div class="empty">Não existem encomendas para esta data.</div>'}<div class="footer"><span>VitalVeg · Central Inteligente</span><span>Data de entrega ${esc(deliveryDate)}</span></div><div class="ai-note">Documento operacional gerado a partir das encomendas consolidadas pela Central. Linhas sinalizadas devem ser confirmadas antes da confirmação.</div>`;
    loadLogo();
